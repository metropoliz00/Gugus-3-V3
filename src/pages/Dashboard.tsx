...
const adminMenu = [
  ...
  { id: "pengaturan_akun", label: "Pengaturan Akun", icon: Users },
];
                ...
                <Routes>
                  ...
                  {user.role?.toLowerCase() === "admin" && (
                    ...
                    <Route path="sekolah" element={<AdminSekolahForm />} />
                      ...
                      <Route
                        path="finance"
                        element={<AdminFinanceManagement />}
                      />
...
function UserSettings({ user }: { user: User }) {
  ...
  const handleUpdate = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch(`/api/v1/update-user?t=${Date.now()}`, {
        method: "POST",
      ...
      setMsg({
        type: "success",
        text: "Profil berhasil diperbarui! Silakan refresh halaman atau login ulang untuk melihat perubahan penuh.",
      });
      await alert("Profil berhasil diperbarui!");
    } catch (err: any) {
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-8">
      <div className="bg-white/80 backdrop-blur-xl p-8 rounded-3xl border border-main-orange/20 shadow-xl">
        <h2 className="text-2xl font-bold font-heading text-soft-black mb-2">
          Pengaturan Akun
        </h2>
        <p className="text-gray-500 text-sm mb-8">
          Perbarui informasi personal dan keamanan akun Anda.
        </p>
        ...
        <form onSubmit={handleUpdate} className="space-y-8">
          <div className="flex flex-col md:flex-row items-center gap-8 pb-8 border-b border-gray-100">
            ...
            <div className="flex-1 space-y-2 text-center md:text-left">
              <h3 className="text-xl font-bold text-soft-black">
                {formData.nama || formData.username}
              </h3>
              <p className="text-main-blue font-medium px-3 py-1 bg-blue-50 rounded-full inline-block text-xs uppercase tracking-wider">
                {formData.role}
              </p>
              <p className="text-gray-400 text-xs mt-2 italic">
                Unggah foto formal untuk profil Anda di sistem.
              </p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="space-y-1">
              ...
              <input
                disabled
                className="w-full bg-gray-50 border border-gray-200 p-3 rounded-xl outline-none text-gray-400 cursor-not-allowed"
          ...
          {msg.text && (
            <div
              className={`p-4 rounded-xl text-sm ${msg.type === "success" ? "bg-green-50 text-green-700" : "bg-red-50 text-red-700"}`}
            >
              {msg.text}
            </div>
          )}
          ...
          <div className="pt-4 flex justify-end">
            <button
              ...
              className="px-8 py-4 bg-gradient-to-r from-main-blue to-blue-600 text-white rounded-2xl font-bold shadow-xl shadow-blue-500/20 hover:scale-[1.02] active:scale-[0.98] transition-all disabled:opacity-50"
            >
      ...
      try {
        if (!responseText) {
          ...
          if (response.status === 404) {
            throw new Error(
              "Endpoint API tidak ditemukan (404). Periksa konfigurasi server.",
            );
          } else if (response.status >= 500) {
            throw new Error(
              `Server mengalami masalah (HTTP ${response.status}). Coba lagi nanti.`,
            );
          } else {
            throw new Error(
              `Sistem tidak memberikan respon (HTTP ${response.status}).`,
            );
          }
        }
        ...
        try {
          ...
          throw new Error(
            `Respon server bukan format JSON valid. (Detail: ${responseText.substring(0, 30)}...)`,
          );
        }
        ...
        if (!response.ok) {
          throw new Error(
            result.error ||
              result.message ||
              `Gagal memproses user (HTTP ${response.status})`,
          );
        }
      } catch (innerErr: any) {
        throw innerErr;
  ...
  const handleDelete = async (id: string, name: string) => {
    ...
    try {
      const response = await fetch(
        `/api/v1/delete-user/${id}?t=${Date.now()}`,
        {
          method: "DELETE",
        },
      );
    ...
    <div className="space-y-8">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white/80 backdrop-blur-xl p-8 rounded-3xl border border-main-orange/20 shadow-xl shadow-blue-500/5">
        ...
        <div className="flex flex-wrap gap-3">
          ...
          <label
            title="Upload Excel Massal"
            className="p-3 bg-green-600 text-white flex items-center justify-center font-bold rounded-xl hover:bg-green-700 transition-all shadow-lg shadow-green-500/20 cursor-pointer"
          >
            {isUploading ? (
              ...
              <tbody className="divide-y divide-gray-50">
                ...
                {filteredUsers.map((usr, i) => (
                  ...
                  >
                    ...
                    <td className="px-6 py-4">
                      <div
                        className="text-xs font-bold text-gray-500 truncate max-w-[150px]"
                        title={usr.email}
                      >
                        {usr.email || "-"}
                      </div>
...
function AdminOverview() {
  ...
  useEffect(() => {
    const fetchStats = async () => {
      ...
      try {
        const [postRes, docRes, eventRes, userRes, schoolRes] =
          await Promise.all([
            supabase
              .from("posts")
            ...
            {[
              {
                ...
                desc: "Ahmad mengunggah RPP Matematika",
                icon: UploadCloud,
                color: "bg-blue-500",
              },
              {
                time: "1 Jam lalu",
                title: "Berita Baru",
                desc: 'Budi mempublikasikan "Kegiatan KKG"',
                icon: FileText,
                color: "bg-green-500",
              },
              {
                ...
                desc: "Indah mendaftar sebagai guru baru",
                icon: Users,
                color: "bg-purple-500",
              },
              {
                time: "5 Jam lalu",
                title: "Pengumuman",
                desc: "Sistem OTOMATIS: Jadwal Maintenance",
                icon: Bell,
                color: "bg-amber-500",
              },
            ].map((act, i) => (
              ...
              >
                <div
                  className={`flex items-center justify-center w-6 h-6 rounded-full border-4 border-white ${act.color} text-white shrink-0 md:order-1 md:group-odd:-translate-x-1/2 md:group-even:translate-x-1/2 shadow flex-col absolute left-0 md:left-1/2 md:-translate-x-1/2 z-10`}
                />
...
function GuruOverview() {
  ...
  const activities = [
    ...
    {
      title: "Tugas Dikoreksi",
      icon: CheckSquare,
      color: "from-green-500 to-emerald-400",
      value: "Monitoring Tugas",
    },
    {
      title: "Agenda KKG",
      icon: Users,
    ...
    <div className="space-y-8">
      ...
      >
        ...
        <div className="relative z-10 max-w-2xl">
          ...
          <p className="text-blue-50 text-lg md:text-xl font-light mb-8">
            Platform terintegrasi untuk administrasi, berbagi perangkat ajar,
            dan informasi kegiatan Gugus.
          </p>
...
function AdminAgendaForm() {
  ...
  const handleUpdate = async (id: string, updates: any) => {
    ...
    if (!error) {
      setEvents(
        events.map((g: any) => (g.id === id ? { ...g, ...updates } : g)),
      );
    }
  };
...
function AdminPenghargaanForm() {
  ...
  const handleUpdate = async (id: string, updates: any) => {
    ...
    if (!error) {
      setAwards(
        awards.map((a: any) => (a.id === id ? { ...a, ...updates } : a)),
      );
    }
  };
